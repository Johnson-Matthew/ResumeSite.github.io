...
var Un = ((r, t) => {
  ...
  const b = t, f = useSlots(), q = inject(dn, {}), E = r, e = ref({ ...E, ...q });
  let L, A;
  const x = ref(null), h = ref("copy"), T = ref(""), F = ref(false), I = ref(false), C = ref(false), kn = ref("https://cdn.jsdelivr.net/gh/highlightjs/cdn-release@11.8.0/build/styles"), Q = ref("https://cdn.jsdelivr.net/gh/PrismJS/prism@1.29.0/themes"), fn = ref("https://cdn.jsdelivr.net/gh/PrismJS/prism-themes@1.9.0/themes"), M = ref(""), G = ref(""), j = ref(""), { copyButton: _n, copyIcons: wn, copyTab: X, cssPath: J, label: Z, runTab: nn, tabs: N } = toRefs(e.value), xn = computed(() => {
    let a = "";
    return e.value.highlightjs && (a = M.value), e.value.prismjs && e.value.prismPlugin && (a = x.value), a;
  }), Tn = computed(() => ((a) => {
    const { isMobile: s, isPrism: o } = a;
    return { [`${u}`]: true, [`${u}-mb-5`]: true, [`${u}--mobile`]: unref(s), [`${u}--prismjs`]: unref(o), [`${u}--highlightjs`]: !unref(o) };
  })({ isMobile: C, isPrism: e.value.prismjs })), $n = computed(() => ((a) => {
    const { copyStatus: s, isMobile: o, persistentCopyButton: l } = a;
    return { [`${u}--code-copy-button`]: true, [`${u}--code-copy-button-mobile`]: unref(o), [`${u}--code-copy-button-persist`]: unref(l), [`${u}--code-copy-button-status-${unref(s)}`]: true };
  })({ copyStatus: h, isMobile: C, persistentCopyButton: e.value.persistentCopyButton })), en = computed(() => ((a) => {
    const { copyStatus: s, highlightjs: o, useTheme: l } = a, y = bn(unref(o)), v = pn(unref(l));
    return { [`${u}--button-copy-icon-status-${unref(s)}`]: true, [`${u}--me-1`]: true, [`${u}--tab-${y}-${v}-icon-status-${unref(s)}`]: true, [`${u}--tab-${y}-${v}-icon`]: true };
  })({ copyStatus: h, highlightjs: e.value.highlightjs, useTheme: j })), Cn = computed(() => ((a) => {
    const { isMobile: s } = a;
    return { [`${u}--label`]: true, [`${u}--label-mobile`]: unref(s) };
  })({ isMobile: C })), an = computed(() => ((a) => {
    const { highlightjs: s, useTheme: o } = a, l = bn(unref(s)), y = pn(unref(o));
    return { [`${u}--tab-${y}`]: true, [`${u}--tab-${l}-${y}`]: true };
  })({ highlightjs: e.value.highlightjs, useTheme: j })), ln = computed(() => ((a) => {
    const { isLoading: s, useTheme: o } = a, l = { width: unref(o) === "coy" && unref(s) === false ? "100%" : "" };
    return unref(o) === "coy" && (l.padding = "1em"), l;
  })({ isLoading: I, useTheme: j })), zn = computed(() => ((a) => {
    const { floatingTabs: s, tabGap: o } = a;
    return { bottom: s ? "1px" : "0", gap: W({ str: unref(o) }) || "0px" };
  })({ floatingTabs: e.value.floatingTabs, tabGap: e.value.tabGap })), Sn = computed(() => ((a) => {
    const { copyTab: s, height: o, maxHeight: l, radius: y, runTab: v, tabs: z, useTheme: S } = a, p = unref(y);
    let _ = `${p} 0 ${p} ${p} !important`;
    return unref(z) && (unref(s) || unref(v)) || (_ = p), { borderRadius: _, display: unref(S) !== "funky" ? "flex" : "block", height: W({ str: unref(o) }), maxHeight: W({ str: unref(l) }), overflow: "auto" };
  })({ copyTab: X.value, height: e.value.height, maxHeight: e.value.maxHeight, radius: e.value.codeBlockRadius, runTab: nn.value, tabs: N.value, useTheme: j })), Bn = computed(() => ((a) => {
    const { tabGap: s } = a;
    return { gap: W({ str: unref(s) }) || "0px" };
  })({ tabGap: e.value.tabGap }));
  function sn() {
    if (!e.value.prismjs && !e.value.highlightjs)
    ...
    if (e.value.highlightjs && e.value.prismPlugin)
  ...
  function O() {
    let a = "";
    const s = e.value.highlightjs ? "highlightjs" : "prism", o = document.getElementsByTagName("head")[0], l = document.createElement("style"), y = document.body.getAttribute("data-v-code-block-theme");
    let v = `v-code-block--theme-${j.value}-${s}`, z = true, S = true, p = "", _ = "";
    if (v = rn(s, v), y === j.value || y === v || typeof j.value == "boolean")
      return;
    switch (document.body.setAttribute("data-v-code-block-theme", v), l.setAttribute("type", "text/css"), l.setAttribute("data-theme-id", v), l.setAttribute("data-theme", "v-code-block--theme-sheet"), j.value) {
      case "neon-bunny":
        a = `/**
	* MIT License
...
`, S = false, z = false, s === "highlightjs" && (a = `/**
	* MIT License
	* Copyright (c) 2023 WebDevNerdStuff
	* WebDevNerdStuff Neon Bunny
	* VSCode Theme: https://marketplace.visualstudio.com/items?itemName=WebDevNerdStuff.neon-bunny
...
`);
    ...
    if (!S && !z)
      return tn(), l.appendChild(document.createTextNode(a)), void o.appendChild(l);
    const gn = rn(s, j.value);
    if (J.value !== void 0)
      _ = J.value;
    else
      switch (s) {
        case "highlightjs":
          p = `${gn}.min.css`, _ = `${kn.value}/${p}`;
          break;
        case "prism":
          p = `${gn}.css`, _ = j.value === "default" ? `${Q.value}/prism.css` : j.value.includes("themes-") ? `${fn.value}/${p}` : `${Q.value}/prism-${p}`;
          break;
        default:
          p = "";
      }
    I.value = true, fetch(_).then((B) => B.text()).then((B) => {
      tn(), l.appendChild(document.createTextNode(B)), o.appendChild(l), I.value = false;
    }).catch((B) => {
      const qn = e.value.highlightjs ? "Highlight.js" : "PrismJS";
      console.error(`${qn} CDN Error:`, B);
    });
  }
  function on() {
  ...
  function rn(a, s, o = false) {
    let l = s;
    return a === "prism" && (l = l.replace("themes-", "prism-"), l = l.replace("prism-prism-", "prism-"), l = l.replace("theme-prism-", "theme-"), o && (l = l.replace("prism-prism-", ""), l = l.replace("prism-", ""))), a === "highlightjs" && (l = l.replace("base16-", "base16/")), l;
  }
  function tn() {
  ...
  function un() {
    (function() {
      if (e.value.lang !== "json")
        x.value = e.value.code;
      else {
        const a = e.value.code.toString();
        x.value = JSON.stringify(JSON.parse(a), null, e.value.indent);
      }
    })(), e.value.highlightjs && import("./es-QEVWC7QB.js").then((a) => {
      L = a.default, L.registerLanguage("plain", Fn), M.value = L.highlight(x.value, { language: e.value.lang }).value;
    }).catch((a) => {
      console.error("Highlight.js import:", { err: a });
    }), e.value.prismjs && import("./prism-YA44QK5O.js").then((a) => {
      A = a.default, M.value = A.highlight(x.value, A.languages[e.value.lang], e.value.lang);
    }).catch((a) => {
  ...
  return watch(E, () => {
    e.value = { ...E, ...q }, e.value.code && un(), (e.value.theme || e.value.prismjs || e.value.highlightjs) && (sn(), j.value = e.value.theme, O()), e.value.copyText && (T.value = e.value.copyText), e.value.runText && (G.value = e.value.runText);
  }), watch(() => J, () => {
    O();
  }), onBeforeMount(() => {
    T.value = e.value.copyText, G.value = e.value.runText, sn();
  }), onMounted(() => {
    j.value = e.value.theme, O(), on(), un();
  }), window.addEventListener("orientationchange", () => {
    on();
  }), (a, s) => (openBlock(), createElementBlock("div", { class: normalizeClass(unref(Tn)) }, [unref(Z) || unref(N) || unref(f).label || unref(f).tabs ? (openBlock(), createElementBlock("div", { key: 0, class: "v-code-block--header", style: normalizeStyle(unref(zn)) }, [createBaseVNode("div", { class: normalizeClass(["v-code-block--label v-code-block--pb-1", unref(Cn)]) }, [unref(f).label ? renderSlot(a.$slots, "label", normalizeProps(mergeProps({ key: 0 }, { copyCode: P, copyStatus: unref(h), runCode: R })), void 0, true) : (openBlock(), createElementBlock(Fragment, { key: 1 }, [createTextVNode(toDisplayString(unref(Z)), 1)], 64))], 2), createBaseVNode("div", { class: "v-code-block--tabs", style: normalizeStyle(unref(Bn)) }, [unref(f).tabs ? renderSlot(a.$slots, "tabs", normalizeProps(mergeProps({ key: 0 }, { copyCode: P, copyStatus: unref(h), runCode: R })), void 0, true) : (openBlock(), createElementBlock(Fragment, { key: 1 }, [unref(X) && unref(N) ? (openBlock(), createElementBlock("div", { key: 0, class: normalizeClass(["v-code-block--tab", unref(an)]), onClick: P }, [createBaseVNode("div", In, [unref(wn) ? (openBlock(), createBlock(yn, { key: 0, class: normalizeClass(["v-code-block--button-copy-icon", unref(en)]), icon: unref(h) }, null, 8, ["class", "icon"])) : createCommentVNode("", true), createTextVNode(" " + toDisplayString(unref(T)), 1)])], 2)) : createCommentVNode("", true), unref(nn) && unref(N) && !unref(C) ? (openBlock(), createElementBlock("div", { key: 1, class: normalizeClass(["v-code-block--tab v-code-block--tab-run", unref(an)]), onClick: R }, [createBaseVNode("div", Gn, toDisplayString(unref(G)), 1)], 2)) : createCommentVNode("", true)], 64))], 4)], 4)) : createCommentVNode("", true), createBaseVNode("div", Jn, [createBaseVNode("div", { class: normalizeClass(["v-code-block--code-copy-button", unref($n)]), onClick: P }, [unref(f).copyButton ? renderSlot(a.$slots, "copyButton", normalizeProps(mergeProps({ key: 0 }, { copyStatus: unref(h) })), void 0, true) : (openBlock(), createElementBlock(Fragment, { key: 1 }, [unref(_n) ? (openBlock(), createBlock(yn, { key: 0, class: normalizeClass(["v-code-block--button-copy-icon", unref(en)]), icon: unref(h) }, null, 8, ["class", "icon"])) : createCommentVNode("", true)], 64))], 2), createBaseVNode("pre", mergeProps(a.$attrs, { class: `language-${unref(e).lang}`, style: unref(Sn) }), [createTextVNode("				"), a.prismPlugin ? (openBlock(), createElementBlock("code", { key: 0, class: normalizeClass(`language-${unref(e).lang} ${unref(e).browserWindow ? "v-code-block--code-browser" : ""} ${unref(e).highlightjs ? "hljs" : ""}`), style: normalizeStyle(unref(ln)), textContent: toDisplayString(unref(xn)) }, null, 14, On)) : (openBlock(), createElementBlock("code", { key: 1, class: normalizeClass(`language-${unref(e).lang} ${unref(e).browserWindow ? "v-code-block--code-browser" : ""} ${unref(e).highlightjs ? "hljs" : ""}`), style: normalizeStyle(unref(ln)), innerHTML: unref(M) }, null, 14, Rn)), createTextVNode(`
			`)], 16)])], 2));
} }), [["__scopeId", "data-v-9ef440d2"]]);
var vn = Object.freeze(Object.defineProperty({ __proto__: null, default: Un }, Symbol.toStringTag, { value: "Module" }));
var dn = Symbol();
function Qn(r = {}) {
  return { install: (t) => {
    t.provide(dn, r), t.component("CodeBlock", defineAsyncComponent(() => Promise.resolve().then(() => vn))), t.component("VCodeBlock", defineAsyncComponent(() => Promise.resolve().then(() => vn)));
  } };
}
(function() {
  "use strict";
